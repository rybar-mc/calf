...
use super::utils::{extract_head_png, fetch_image_bytes, get_decoded_textures, resolve_uuid};

#[utoipa::path(
    get,
    path = "/v1/players/{identifier}/head",
    responses(
        (status = 200, description = "Player head in png"),
...
pub async fn get_player_head(_req: Request, ctx: RouteContext<()>) -> Result<Response> {
    ...
    let head_png = match extract_head_png(&skin_bytes)? {
        Ok(h) => h,
        Err(e) => return Ok(e),
    };

    let mut response = Response::from_bytes(head_png)?;
